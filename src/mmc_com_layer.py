...
from src.logger import logger, custom_logger, _unify_external_loggers
...
async def mmc_start_com():
    ...
    if config.enable_api_server and HAS_MESSAGE_CONVERTER:
        ...
        # WebSocketClient 初始化时可能重新配置 maim_message logger 的 handler，
        # 再次统一以确保所有日志走 echotools 格式
        _unify_external_loggers()
        message_send_instance.maibot_router = router
        # 连接就绪，通知消息处理循环可以开始消费
        from main import mmc_ready
        mmc_ready.set()
        await router.run()
        ...
        # Router.__init__() 调用 setup_logger(external_logger=...) 替换了 _logger，
        # 但 maim_message stdlib logger 可能仍保留旧 handler，再次统一
        _unify_external_loggers()
        router.register_class_handler(send_handler.handle_message)
        message_send_instance.maibot_router = router
        # 路由已配置，通知消息处理循环可以开始消费
        from main import mmc_ready
        mmc_ready.set()
        await router.run()
