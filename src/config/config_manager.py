...
class ConfigManager:
    ...
    def load(self, config_path: str = "config.toml") -> None:
        ...
        _get_logger().info(f"配置已加载: {config_path}")
    ...
    async def reload(self, config_path: Optional[str] = None) -> bool:
        ...
        async with self._lock:
            ...
            try:
                ...
                _get_logger().info(f"配置重载成功: {config_path}")
                return True
                
            except Exception as e:
                _get_logger().error(f"配置重载失败: {e}", exc_info=True)
                return False
    ...
    async def start_watch(self) -> None:
        ...
        if self._observer is not None:
            _get_logger().warning("配置文件监控已在运行")
            return
        ...
        class ConfigFileHandler(FileSystemEventHandler):
            ...
            def on_modified(handler_self, event):
                ...
                if isinstance(event, FileModifiedEvent) and os.path.abspath(event.src_path) == handler_self.config_path:
                    _get_logger().debug(f"检测到配置文件变更: {event.src_path}")
                    # 使用防抖机制避免重复重载
        ...
        _get_logger().info(f"已启动配置文件实时监控: {self._config_path}")
    ...
    async def _debounced_reload(self) -> None:
        ...
        if self._last_reload_trigger > trigger_time:
            # 有更新的触发，放弃本次重载
            _get_logger().debug("放弃过时的重载请求")
            return
        
        # 检查是否已有重载在进行
        if self._is_reloading:
            _get_logger().debug("重载已在进行中，跳过")
            return
